...
- **v1.5.1**：统一仓库文档与目录，合并两个早期运行时热修复，并保留旧存档军路校正兼容。
- **v1.5.0**：加入季节与动态战线，并把阶段目标、人物差遣和前线情报集中到统一战役台。
- **v1.4.0**：人物可巡抚州郡、持节出使或监军劳师，结果直接作用于既有系统。
- **v1.3.0**：五个剧本各加入四段清晰目标，并以小幅奖励引导不同开局策略。
- **v1.2.0**：地图可直接下达军团命令，总览压缩为最多三条关键警讯。
...
- 完整存档覆盖核心游戏、月报、天下、诏令、军略、军团、政议、音频、剧本、御前档案和战役演进。
...
```bash
EXPECTED_VERSION=1.5.1 node tests/release-regression.js
```
