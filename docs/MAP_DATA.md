...
v2.6.1 的海岸线与岛屿轮廓继续使用 [Natural Earth](https://www.naturalearthdata.com/) 1:110m 数据简化、裁切和等距经纬投影而来。Natural Earth 地图数据属于公有领域。
...
- 14 组大号州域名称由 `regionLabels` 维护；32 处郡国与治所标注由 `minorPlaces` 维护，均使用同一经纬度投影。
- 地图不用于导航、行政界定或学术测绘。
...
- 州名与郡国治所：修改 `regionLabels` 与 `minorPlaces`。
